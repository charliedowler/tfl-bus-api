...
var Bus = function(bus, callback) {
  ...
  request({
    ...
    _.each(this.data.directions[1].segments[0].markers, function(busStop) {
...
Bus.prototype.findStopBySmsCode = function(smsCode) {
  return _.findWhere(this.data.directions[1].segments[0].markers, { smsCode: smsCode });
};

Bus.prototype.findStopByName = function(name) {
  return _.findWhere(this.data.directions[1].segments[0].markers, { name: name });
};

Bus.prototype.filterByDirection = function(dir) {
  return _.findWhere(this.data.directions[1].segments[0].markers, { direction: dir });
};
